...
def day(english_year, english_month, english_day):
    ...
    if english_days < 50:
        ...
        return d49
    chinese_year_date_list = helper_year_data_find(english_days, chinese_years_days)  # 获得年位置 [33271, 91, 1992, 16]

    change_dict = {'00': 29, '01': 30, '10': 29, '11': 30, }
    a = bin(CHINESE_1990_2100[chinese_year_date_list[1]])
    ...
    chinese_month_list = helper_month_data_find(chinese_year_date_list[3], d)  # 获得月位置 [0, 0, 1, 17]
    ...
    # 天干地支的计算
    # 计算立春的天数
    spring_days_count = helper_days_60_list()
    # 所属年的起始天数，第几年，农历年份，年内距离立春的天数
    spring_day_count_data_list = helper_year_60_data_find(english_days, spring_days_count)

    # 解析当年24气节的数据
    holiday_data_str = bin(CHANGE_1990_2100_24[spring_day_count_data_list[1]])
    holiday_data_str = holiday_data_str[3:]
    holiday_index_data_list = [holiday_data_str[i:i + 2] for i in range(0, len(holiday_data_str), 2)]  # 去除保留数值
    con_holiday = [holiday_index_data_list[i] for i in range(2, len(holiday_index_data_list), 2)]
    a1 = {'00': '2', '01': '3', '10': '4', '11': '5', }
    a2 = {'00': '4', '01': '5', '10': '6', '11': '7', }
    a3 = {'00': '9', '01': '6', '10': '7', '11': '8', }
    # 拿到所有
    user_year_holiday_data = [a1[con_holiday[0]], a2[con_holiday[1]], a2[con_holiday[2]], a2[con_holiday[3]],
                              a2[con_holiday[4]], a3[con_holiday[5]], a3[con_holiday[6]], a3[con_holiday[7]],
                              a3[con_holiday[8]], a3[con_holiday[9]], a3[con_holiday[10]], ]  # 转换为１０进制的列表

    e2 = bin(CHANGE_1990_2100_24[spring_day_count_data_list[1] + 1])  # 获取向前一年的u"小寒",u"大寒"
    e2 = e2[3:]
    f2 = [e2[i:i + 2] for i in range(0, len(e2), 2)]
    g2 = [f2[i] for i in range(0, len(f2), 2)]
    user_year_holiday_data = user_year_holiday_data + [a2[g2[0]]]

    user_year_holiday_count = []
    # 所有农历节气的日期距离(到当年立春)
    user_year_spring_date = datetime.date(spring_day_count_data_list[2], 2, int(user_year_holiday_data[0]))
    for v in range(11):
        v_day = datetime.date(spring_day_count_data_list[2], 2 + v,
                              int(user_year_holiday_data[v])) - user_year_spring_date
        user_year_holiday_count.append(v_day.days)
    # 下一年小寒
    v_day_2 = datetime.date(spring_day_count_data_list[2] + 1, 1,
                            int(user_year_holiday_data[11])) - user_year_spring_date
    user_year_holiday_count.append(v_day_2.days)
    # 生成月干支天数，数据列表，spring_day_count_data_list[3]是距离当年立春天数
    month_data_list = helper_month_60_data_find(spring_day_count_data_list[3],
                                                user_year_holiday_count)  # 获得月干支位置[60, 2, 3, 1]

    branches_list = CHINESE_60 + CHINESE_60 + CHINESE_60 + CHINESE_60 + CHINESE_60
    branches_list = branches_list[37:37 + 200]  # 生成年干支列表
    year_value_key = int(branches_list[spring_day_count_data_list[1]][0:2])  # 年干
    ...
    f2 = str(spring_day_count_data_list[2]) + '-' + str(chinese_month_list_2) + '-' + str(chinese_month_list[3])
    f3 = CHINESE_10_1[int(branches_list[spring_day_count_data_list[1]][0:2])] + CHINESE_12_1[
        int(branches_list[spring_day_count_data_list[1]][2:4])] + '-' + v2[month_data_list[1]] + v3[
             month_data_list[1]] + '-' + CHINESE_10_1[int(CHINESE_60[v5][0:2])] + CHINESE_12_1[
             int(CHINESE_60[v5][2:4])]
    f4 = CHINESE_12_2[int(branches_list[spring_day_count_data_list[1]][2:4])]  # 生肖
